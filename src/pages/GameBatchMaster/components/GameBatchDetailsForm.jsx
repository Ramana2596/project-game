...
/**
 * Component: GameBatchDetailsForm
 * Purpose:
 ...
 */
export default function GameBatchDetailsForm({
  ...
  /** Save form -> inject Game_Id + Game_Batch, then call parent callback */
  const handleSave = () => {
    //  include Game_Batch always
    const updatedForm = { ...form, Game_Id: gameId, Game_Batch: details?.Game_Batch };
    onSave(updatedForm); //  safe call
    setEditMode(false);
  };
...
{/*
                  {Array.isArray(selectOptions[columnName]) && selectOptions[columnName].length > 0 ? (
                    selectOptions[columnName].map((opt, idx) => {
                      if (!opt || typeof opt.value === "undefined") return null;
                      return (
